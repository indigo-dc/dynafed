...
{
  Info(Logger::Lvl1, "FilterNoLoopPlugin", "Filter NoLoopPlugin loaded");
}
...
static void callback_resolve_query(std::vector<Asio_ip::address> & vec, const std::string & hostname,
    ...
    if (ec){
         Info(Logger::Lvl3, "FilterNoLoopPlugin::callback_resolve_query", "Error during resolution " << ec);
         return;
    }
    Resolver::iterator end;
    for (; iter != end; ++iter) {
        Info(Logger::Lvl1, "FilterNoLoopPlugin::callback_resolve_query", " resolution " << hostname << " to " << iter->endpoint().address());
...
static bool is_matching_address(const std::vector< std::vector<Asio_ip::address> > & rep_vec,
    ...
    if(std::find_first_of(rep_vec[*i].begin(), rep_vec[*i].end(), cli_vec.begin(), cli_vec.end()) != rep_vec[*i].end()){
        Info(Logger::Lvl1, "FilterNoLoopPlugin::callback_resolve_query", " Loop detected  on " << elem.name << " deletion ");
        res = true;
    }
    (*i)++;
    return res;
...
static int filter_internal_list(std::deque<UgrFileItem_replica> &replica, const std::vector< std::vector<Asio_ip::address> > & rep_vec,
    ...
    int i=0;
    Info(Logger::Lvl1, "FilterNoLoopPlugin::is_matching_address", " size of replicas " << replica.size() << " size of rep vec" << rep_vec.size() << " size of cli_vec" << cli_vec.size());
...
int FilterNoLoopPlugin::filterReplicaList(std::deque<UgrFileItem_replica> &replica, const UgrClientInfo &cli_info){
    ...
    for(std::deque<UgrFileItem_replica>::iterator it = replica.begin(); it != replica.end(); ++it){
       ...
       if(uri.getStatus() != Davix::StatusCode::OK){
            Info(Logger::Lvl1, "FilterNoLoopPlugin::filterReplicaList", "Invalid replica content " << it->name);
            continue;
       }
