...
void UgrLocPlugin_Azure::configure_Azure_parameters(const std::string & prefix){
    ...
    if ( signature_validity < (time_t)UgrCFG->GetLong("extcache.memcached.ttl", 1000000L)-60 ) {
    ...
    if ( signature_validity < (time_t)UgrCFG->GetLong("infohandler.itemmaxttl", 1000000L)-60 ) {
      Error(name, " The given signature validity of " << signature_validity <<
