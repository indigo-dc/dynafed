...
int UgrFileInfo::waitStat(boost::unique_lock<boost::mutex> &l, int sectmout) {
    ...
    Info(SimpleDebug::kHIGH, fname, "Finished check-wait. Name: " << name << " Status: " << getStatStatus() << " status_statinfo: " << status_statinfo);
...
const char *fname = "UgrFileInfo::waitLocations";
    ...
    Info(SimpleDebug::kHIGH, fname, "Finished check-wait. Name: " << name << " Status: " << getItemsStatus() << " status_items: " << status_items);

    // We are here if the plugins have finished OR in the case of timeout
    // If the loc is still marked as in progress it means that some plugin is very late.
    if ((getLocationStatus() == InProgress) && (status_items == NoInfo))
