...
\subsubsection{glb.filepullhook.usereplicaurl}
 In the case of a "pull" third party copy, pass to the helper script a destination URL pointing to an endpoint, instead of passing as destination a path that is local to this Ugr instance (logical file name).
  
 Syntax:\\
 
 \lstinline"glb.filepullhook.usereplicaurl:  <true | false>"\\
...
\subsubsection{locplugin.$<$ID$>$.candochecksum}
Set this to true if the plugin can be directly queried for checksums by a client that is redirected to it.
Syntax:\\
\lstinline"locplugin.<ID>.candochecksum: <true|false>"\\
Default value: false\\
...
\begin{itemize}
 ...
 \item \lstinline"destURL" is the full URL of the destination file or the local logical file name in case of a pull copy
