...
\subsubsection{locplugin.$<$ID$>$.s3.v2alternate}
When using the default S3 signing algorithm v2, the URL prefix will have to be similar to the following example:\\

\lstinline"s3://mybucket.s3.amazonaws.com"\\

 where the bucket name appears in the hostname part of the URL.\\
 
 If the boolean parameter \lstinline"v2alternate" is set to true, the Dynafed S3 plugin
 will expect an alternate syntax for that URL, which is similar to:\\
 
\lstinline"s3://s3.amazonaws.com/mybucket"\\

Syntax:\\
\lstinline"locplugin.<ID>.s3.v2alternate: <yes|true>"\\
